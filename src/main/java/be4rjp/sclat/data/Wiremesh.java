            ...
            for (Player player : Main.getPlugin().getServer().getOnlinePlayers()) {
                ...
                if (block.getLocation().distance(player.getLocation()) <= 5) {
                    ...
                    if (!playerList.contains(player)) {
                        PacketPlayOutSpawnEntity packet = new PacketPlayOutSpawnEntity(fb, 70, net.minecraft.server.v1_13_R2.Block.getCombinedId(ibd));
                    ...
                    if (playerList.contains(player)) {
                        PacketPlayOutEntityDestroy packet = new PacketPlayOutEntityDestroy(fb.getBukkitEntity().getEntityId());
                        entityPlayer.playerConnection.sendPacket(packet);
                        player.sendBlockChange(block.getLocation(), blockData);
